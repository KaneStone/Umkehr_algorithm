...
for j = 1:length(lambda);
    for iscat = 1:atmos.nlayers-1;
        ...
        intensity(j,:,iscat) = (intensity(j,:,iscat).*...
            ...
            ozonexs(j,1:end-1).*atmos.ozonemid(:)')*...
...
%This works motherfucker
%a = intensity(j,:,iscat).*exp(-1.*(atmos.bRay(j,:)+atmos.bMie(j,:)+ozonexs(j,1:end-1).*atmos.ozonemid(:)')*squeeze(zs(j,:,iscat,:))'.*100);
